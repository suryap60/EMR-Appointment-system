...
const Patients: React.FC = () => {
    ...
    const loadPatients = async () => {
        setLoading(true);
        try {
            const res = await patientService.getPatients({ search, page });
        ...
        <div className="max-w-6xl mx-auto space-y-6 fade-in pb-20">
            <header className="flex flex-col md:flex-row md:items-end justify-between border-b border-slate-200 pb-4 gap-4">
                <div>
                    ...
                    <p className="text-slate-500 mt-1.5">View and search through all registered patients in the system.</p>
                </div>
            </header>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left text-sm whitespace-nowrap">
                        ...
                        <tbody className="divide-y divide-slate-100">
                            {loading ? (
                                <tr>
                                    <td colSpan={4} className="py-10 text-center text-slate-400">Loading patients...</td>
                                </tr>
                            ) : patients.length === 0 ? (
                                <tr>
